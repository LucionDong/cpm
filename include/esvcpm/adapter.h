...
static inline void neu_req_subscribe_groups_fini(neu_req_subscribe_groups_t *req) {
...
inline static UT_icd *neu_resp_get_sub_driver_tags_info_icd() {
    static UT_icd icd = {sizeof(neu_resp_get_sub_driver_tags_info_t), NULL, NULL, NULL};
    return &icd;
}
...
static inline void neu_reqresp_template_group_fini(neu_reqresp_template_group_t *grp) {
...
static inline void neu_req_add_template_tag_fini(neu_req_add_template_tag_t *req) {
...
static inline void neu_req_del_template_tag_fini(neu_req_del_template_tag_t *req) {
    for (uint16_t i = 0; i < req->n_tag; ++i) {
        free(req->tags[i]);
    }
    free(req->tags);
...
static inline void neu_req_ndriver_tag_param_fini(neu_req_ndriver_tag_param_t *p) {
    free(p->name);
    free(p->params);
}
...
static inline void neu_req_update_ndriver_tag_param_fini(neu_req_update_ndriver_tag_param_t *p) {
...
static inline void neu_req_update_ndriver_tag_info_fini(neu_req_update_ndriver_tag_info_t *p) {
    for (uint16_t i = 0; i < p->n_tag; ++i) {
...
typedef int (*neu_adapter_update_metric_cb_t)(neu_adapter_t *adapter, const char *metric_name, uint64_t n,
                                              const char *group);
typedef int (*neu_adapter_register_metric_cb_t)(neu_adapter_t *adapter, const char *name, const char *help,
                                                neu_metric_type_e type, uint64_t init);

typedef struct adapter_callbacks {
    int (*command)(neu_adapter_t *adapter, neu_reqresp_head_t head, void *data);
    int (*response)(neu_adapter_t *adapter, neu_reqresp_head_t *head, void *data);
    ...
    union {
        struct {
            ...
            void (*update_with_meta)(neu_adapter_t *adapter, const char *group, const char *tag, neu_dvalue_t value,
                                     neu_tag_meta_t *metas, int n_meta);
            void (*write_response)(neu_adapter_t *adapter, void *req, int error);
            void (*update_im)(neu_adapter_t *adapter, const char *group, const char *tag, neu_dvalue_t value,
                              neu_tag_meta_t *metas, int n_meta);
        } driver;
        struct {
            ...
            int (*uart_frame_arrived)(neu_adapter_t *adapter, const uart_frame_t *msg);
