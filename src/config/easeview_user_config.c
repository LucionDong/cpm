...
static int query_uart_config_from_db(esv_uart_configs_t **configs_result) {
    ...
    /* const char *query ="SELECT value FROM easeview_config WHERE group_name = 'uart' and name IN ('uart1', 'uart2',
     * 'uart3', 'uart4')"; */
    const char *query_sql =
        "SELECT device_config FROM easeview_user_config WHERE product_key = 'ESMGTWEV2RYET1KX1R44' LIMIT 1 ";
    ...
    if (SQLITE_ROW == step) {
        ...
        if (root == NULL) {
            nlog_error(
                "easeview user config json load error, line: %d, column: %d, position: %d, info: "
                "%s",
...
int load_uart_config_from_db(UT_array **configs_nodes) {
    ...
    //    while ((p = (char **) utarray_next(configs_nodes, p))) {
    // nlog_info("uart_config: %s", *p);
    // }
