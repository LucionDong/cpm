...
static int adapter_command(neu_adapter_t *adapter, neu_reqresp_head_t header, void *data);
static int adapter_response(neu_adapter_t *adapter, neu_reqresp_head_t *header, void *data);
static int adapter_register_metric(neu_adapter_t *adapter, const char *name, const char *help, neu_metric_type_e type,
                                   uint64_t init);
static int adapter_update_metric(neu_adapter_t *adapter, const char *metric_name, uint64_t n, const char *group);
inline static void reply(neu_adapter_t *adapter, neu_reqresp_head_t *header, void *data);
inline static void notify_monitor(neu_adapter_t *adapter, neu_reqresp_type_e event, void *data);
...
#define REGISTER_METRIC(adapter, name, init) adapter_register_metric(adapter, name, name##_HELP, name##_TYPE, init);

#define REGISTER_DRIVER_METRICS(adapter)                                               \
    REGISTER_METRIC(adapter, NEU_METRIC_LINK_STATE, NEU_NODE_LINK_STATE_DISCONNECTED); \
    REGISTER_METRIC(adapter, NEU_METRIC_RUNNING_STATE, NEU_NODE_RUNNING_STATE_INIT);   \
    REGISTER_METRIC(adapter, NEU_METRIC_LAST_RTT_MS, NEU_METRIC_LAST_RTT_MS_MAX);      \
...
static int adapter_command(neu_adapter_t *adapter, neu_reqresp_head_t header, void *data) {
    ...
    nlog_info("adapter %s recv msg from: %s to %s, type: %s", adapter->name, header.sender, header.receiver,
    ...
    nlog_info("adapter %s gen msg from: %s to %s, type: %s", adapter->name, header.sender, header.receiver,
              neu_reqresp_type_string(header.type));
...
static int adapter_loop(enum neu_event_io_type type, int fd, void *usr_data) {
    ...
    if (type != NEU_EVENT_IO_READ) {
        nlog_warn("adapter: %s recv close, exit loop, fd: %d", adapter->name, fd);
        return 0;
    }

    nng_recvmsg(adapter->sock, &msg, 0);
    header = (neu_reqresp_head_t *) nng_msg_body(msg);
    nlog_info("adapter(%s) recv msg from: %s, type: %s", adapter->name, header->sender,
              neu_reqresp_type_string(header->type));
    ...
    switch (header->type) {
        ...
        case NEU_REQ_UPDATE_TAG_EVENT:
            adapter->module->intf_funs->request(adapter->plugin, (neu_reqresp_head_t *) header, &header[1]);
            break;
        case NEU_REQ_READ_GROUP: {
            neu_resp_error_t error = {0};

            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                neu_adapter_driver_read_group((neu_adapter_driver_t *) adapter, header);
            } else {
        ...
        case NEU_REQ_WRITE_TAG: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                ...
                neu_adapter_driver_write_tag((neu_adapter_driver_t *) adapter, msg_dump);
            } else {
                error.error = NEU_ERR_GROUP_NOT_ALLOW;
                header->type = NEU_RESP_ERROR;
                neu_msg_exchange(header);
        ...
        case NEU_REQ_WRITE_TAGS: {
            ...
            if (adapter->module->type != NEU_NA_TYPE_DRIVER) {
                ...
                void *msg_dump = calloc(nng_msg_len(msg), 1);
                memcpy(msg_dump, header, nng_msg_len(msg));
                neu_adapter_driver_write_tags((neu_adapter_driver_t *) adapter, msg_dump);
            }
            break;
        }
        case NEU_REQ_NODE_SETTING: {
        ...
        case NEU_REQ_GET_GROUP: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                neu_resp_get_group_t resp = {.groups = neu_adapter_driver_get_group((neu_adapter_driver_t *) adapter)};
        ...
        case NEU_REQ_GET_TAG: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                error.error =
                    neu_adapter_driver_query_tag((neu_adapter_driver_t *) adapter, cmd->group, cmd->name, &tags);
            } else {
                error.error = NEU_ERR_GROUP_NOT_ALLOW;
            }
        ...
        case NEU_REQ_ADD_GROUP: {
            ...
            if (cmd->interval < NEU_GROUP_INTERVAL_LIMIT) {
                ...
                if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                    error.error =
                        neu_adapter_driver_add_group((neu_adapter_driver_t *) adapter, cmd->group, cmd->interval);
                } else {
                    error.error = NEU_ERR_GROUP_NOT_ALLOW;
                }
            }
        ...
        case NEU_REQ_UPDATE_DRIVER_GROUP: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                resp.error = neu_adapter_driver_update_group((neu_adapter_driver_t *) adapter, cmd->group,
        ...
        case NEU_REQ_DEL_GROUP: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                error.error = neu_adapter_driver_del_group((neu_adapter_driver_t *) adapter, cmd->group);
            } else {
                error.error = NEU_ERR_GROUP_NOT_ALLOW;
            }
        ...
        case NEU_REQ_DEL_TAG: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                for (int i = 0; i < cmd->n_tag; i++) {
                    int ret = neu_adapter_driver_del_tag((neu_adapter_driver_t *) adapter, cmd->group, cmd->tags[i]);
                    if (0 == ret) {
        ...
        case NEU_REQ_ADD_TAG: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                for (int i = 0; i < cmd->n_tag; i++) {
                    int ret =
                        neu_adapter_driver_validate_tag((neu_adapter_driver_t *) adapter, cmd->group, &cmd->tags[i]);
                    if (ret == 0) {
                        resp.index += 1;
                    } else {
                        resp.error = ret;
            ...
            if (resp.index > 0) {
                int ret =
                    neu_adapter_driver_try_add_tag((neu_adapter_driver_t *) adapter, cmd->group, cmd->tags, resp.index);
                if (ret != 0) {
                    resp.index = 0;
                    resp.error = ret;
                }
            ...
            for (int i = 0; i < resp.index; i++) {
                int ret = neu_adapter_driver_add_tag((neu_adapter_driver_t *) adapter, cmd->group, &cmd->tags[i]);
                if (ret != 0) {
                    neu_adapter_driver_try_del_tag((neu_adapter_driver_t *) adapter, resp.index - i);
                    resp.index = i;
                    resp.error = ret;
                    break;
                }
        ...
        case NEU_REQ_UPDATE_TAG: {
            ...
            if (adapter->module->type == NEU_NA_TYPE_DRIVER) {
                for (int i = 0; i < cmd->n_tag; i++) {
                    int ret =
                        neu_adapter_driver_update_tag((neu_adapter_driver_t *) adapter, cmd->group, &cmd->tags[i]);
                    if (ret == 0) {
        ...
        case NEU_REQ_UPDATE_NDRIVER_TAG_PARAM: {
            neu_req_update_ndriver_tag_param_t *cmd = (neu_req_update_ndriver_tag_param_t *) &header[1];
            neu_resp_update_tag_t resp = {0};
        ...
        case NEU_REQ_UPDATE_NDRIVER_TAG_INFO: {
            neu_req_update_ndriver_tag_info_t *cmd = (neu_req_update_ndriver_tag_info_t *) &header[1];
            neu_resp_update_tag_t resp = {0};
        ...
        case NEU_REQ_GET_NDRIVER_TAGS: {
            neu_req_get_ndriver_tags_t *cmd = (neu_req_get_ndriver_tags_t *) &header[1];
            neu_resp_error_t error = {.error = 0};
            UT_array *tags = NULL;
...
int neu_adapter_start(neu_adapter_t *adapter) {
    ...
    nlog_info("test intf_funs->start");
    if (error == NEU_ERR_SUCCESS) {
...
inline static void reply(neu_adapter_t *adapter, neu_reqresp_head_t *header, void *data) {
    ...
    if (ret != 0) {
        nng_msg_free(msg);
        nlog_warn("%s reply %s to %s, error: %s", header->sender, neu_reqresp_type_string(header->type),
                  header->receiver, nng_strerror(ret));
    }
}
...
void *neu_msg_gen(neu_reqresp_head_t *header, void *data) {
    ...
    switch (header->type) {
        ...
        case NEU_REQRESP_TRANS_DATA: {
            ...
            data_size = sizeof(neu_reqresp_trans_data_t) + trans->n_tag * sizeof(neu_resp_tag_value_meta_t);
            break;
        }
        case NEU_REQ_UPDATE_LICENSE:
