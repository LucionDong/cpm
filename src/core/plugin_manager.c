...
int neu_plugin_manager_add(neu_plugin_manager_t *mgr, const char *plugin_lib_name) {
    char lib_path[256] = {0};
    void *handle = NULL;
    void *module = NULL;
    ...
    if (pm->kind != NEU_PLUGIN_KIND_CUSTOM && pm->kind != NEU_PLUGIN_KIND_SYSTEM) {
        dlclose(handle);
...
UT_array *neu_plugin_manager_get(neu_plugin_manager_t *mgr) {
    ...
    HASH_ITER(hh, mgr->plugins, el, tmp) {
        ...
        strncpy(info.description_zh, el->description_zh, sizeof(info.description_zh));
...
UT_array *neu_plugin_manager_get_single(neu_plugin_manager_t *mgr) {
    ...
    HASH_ITER(hh, mgr->plugins, el, tmp) {
        if (el->single) {
            ...
            strncpy(info.schema, el->schema, sizeof(info.schema));
            strncpy(info.single_name, el->single_name, sizeof(info.single_name));
            strncpy(info.name, el->name, sizeof(info.name));
            strncpy(info.library, el->lib_name, sizeof(info.library));
            strncpy(info.description, el->description, sizeof(info.description));
            strncpy(info.description_zh, el->description_zh, sizeof(info.description_zh));
...
int neu_plugin_manager_find(neu_plugin_manager_t *mgr, const char *plugin_name, neu_resp_plugin_info_t *info) {
    plugin_entity_t *plugin = NULL;
    int ret = -1;
    ...
    if (plugin != NULL) {
        ...
        strncpy(info->description, plugin->description, sizeof(info->description));
    }

    return ret;
}

bool neu_plugin_manager_exists(neu_plugin_manager_t *mgr, const char *plugin_name) {
    plugin_entity_t *plugin = NULL;
    HASH_FIND_STR(mgr->plugins, plugin_name, plugin);
    return NULL != plugin;
}

bool neu_plugin_manager_is_single(neu_plugin_manager_t *mgr, const char *plugin_name) {
    plugin_entity_t *plugin = NULL;
    HASH_FIND_STR(mgr->plugins, plugin_name, plugin);
...
int neu_plugin_manager_create_instance(neu_plugin_manager_t *mgr, const char *plugin_name,
    ...
    if (plugin != NULL) {
        ...
        snprintf(lib_path, sizeof(lib_path) - 1, "%s/%s", g_plugin_dir, plugin->lib_name);
        instance->handle = dlopen(lib_path, RTLD_NOW | RTLD_NODELETE);
        assert(instance->handle != NULL);

        instance->module = (neu_plugin_module_t *) dlsym(instance->handle, "neu_plugin_module");
...
void neu_plugin_manager_load_static(neu_plugin_manager_t *mgr, const char *plugin_name,
                                    neu_plugin_instance_t *instance) {
    (void) mgr;
    instance->handle = NULL;

    if (strcmp(DEFAULT_DASHBOARD_PLUGIN_NAME, plugin_name) == 0) {
        instance->module = (neu_plugin_module_t *) &default_dashboard_plugin_module;
    }
}

void neu_plugin_manager_destroy_instance(neu_plugin_manager_t *mgr, neu_plugin_instance_t *instance) {
    (void) mgr;
    nlog_notice("destroy plugin instance: %s, handle: %p", instance->module->module_name, instance->handle);
...
static void entity_free(plugin_entity_t *entity) {
    nlog_notice("del plugin, name: %s, library: %s, kind: %d, type: %d", entity->name, entity->lib_name, entity->kind,
                entity->type);
    if (entity->single) {
