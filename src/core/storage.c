...
int esv_manager_load_normal_node(neu_manager_t *manager) {
    ...
    nlog_debug("esv_persister_load_normal_nodes begin");
    ...
    utarray_foreach(node_infos, esv_persist_node_info_t *, node_info) {
        rv = neu_manager_add_node(manager, node_info->node_name, node_info->plugin_name, node_info->state, true);
        const char *ok_or_err = (0 == rv) ? "success" : "fail";
        nlog_notice("load adapter %s(%d) type:%d, name:%s plugin:%s state:%d", ok_or_err, rv, node_info->node_type,
                    node_info->node_name, node_info->plugin_name, node_info->state);
    }
...
int esv_manager_load_normal_plugin_from_db(neu_manager_t *manager) {
    ...
    utarray_foreach(plugin_infos, char **, name) {
        nlog_debug("name: %s", *name);  // lib_name
...
int manager_load_node(neu_manager_t *manager) {
    ...
    nlog_debug("neu_persister_load_nodes over");
    if (0 != rv) {
        nlog_error("failed to load adapter infos");
        return -1;
    }
    ...
    utarray_foreach(node_infos, neu_persist_node_info_t *, node_info) {
        rv = neu_manager_add_node(manager, node_info->name, node_info->plugin_name, node_info->state, true);
        const char *ok_or_err = (0 == rv) ? "success" : "fail";
        nlog_notice("load adapter %s type:%d, name:%s plugin:%s state:%d", ok_or_err, node_info->type, node_info->name,
