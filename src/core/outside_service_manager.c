...
nng_msg *esv_thing_model_msg_nng_msg_gen(const esv_thing_model_trans_data_inproc_t *data) {
	...
	/* char* data_json_str = json_dumps(data->data_root, 0); */
	...
	/* memcpy(data_head->sender, "core-plugin-manager", strlen("core-plugin-manager") + 1); */
	/* memcpy(data_head->receiver, "iot-core-service", strlen("iot-core-service") + 1); */
	...
	/* char *ipc_data_json_str = json_dumps(data->data_root, 0); */

	esv_thing_model_trans_data_ipc_t *ipc_data = calloc(1, sizeof(esv_thing_model_trans_data_ipc_t));
	/* memcpy(ipc_data->product_key, data->product_key, strlen(data->product_key) + 1); */
	/* memcpy(ipc_data->device_name, data->device_name, strlen(data->device_name) + 1); */
	...
	/* strcpy((char *)body + sizeof(esv_outside_service_reqresp_head_t) + sizeof(esv_thing_model_trans_data_ipc_t), ipc_data_json_str); */
	if (data_json_size > 0) {
